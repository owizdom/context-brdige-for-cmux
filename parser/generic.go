...
var (
	genericErrorRe    = regexp.MustCompile(`(?mi)(error|exception|failed|traceback)[^\n]{0,120}`)
	genericNoiseRe    = regexp.MustCompile(`(?mi)^\s*time=\d{4}-\d{2}-\d{2}T.*\blevel=\w+.*\bmsg=.*$`)
	genericShellCmdRe = regexp.MustCompile(`(?m)^\s*[^\s]+@[^\s]+\s+[^\s]+\s.*`)
)
...
func (p *GenericParser) Parse(scrollback string) Context {
	...
	for i := len(lines) - 1; i >= 0; i-- {
		...
		if genericNoiseRe.MatchString(line) ||
			strings.Contains(line, "mnemo daemon running") ||
